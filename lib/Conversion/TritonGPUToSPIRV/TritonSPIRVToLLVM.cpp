...
struct SPIRVAddressOfConversion
    : public ConvertOpToLLVMPattern<spirv::AddressOfOp> {
  using ConvertOpToLLVMPattern<spirv::AddressOfOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(spirv::AddressOfOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    auto resultTy = getTypeConverter()->convertType(op.getPointer().getType());

    Value llvmAddressOf =
        rewriter.create<LLVM::AddressOfOp>(loc, resultTy, op.getVariableAttr());

    rewriter.replaceOp(op, llvmAddressOf);

    return success();
  }
};

struct SPIRVUndefConversion : public ConvertOpToLLVMPattern<spirv::UndefOp> {
  using ConvertOpToLLVMPattern<spirv::UndefOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(spirv::UndefOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    auto resultTy = getTypeConverter()->convertType(op.getType());

    Value llvmUndef = rewriter.create<LLVM::UndefOp>(loc, resultTy);

    rewriter.replaceOp(op, llvmUndef);

    return success();
  }
};

struct SPIRVPtrAccessChainConversion
    : public ConvertOpToLLVMPattern<spirv::PtrAccessChainOp> {
  using ConvertOpToLLVMPattern<spirv::PtrAccessChainOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(spirv::PtrAccessChainOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    auto elemPtrTy = getTypeConverter()->convertType(op.getType());
    Value llvmGEP = rewriter.create<LLVM::GEPOp>(
        loc, elemPtrTy, adaptor.getBasePtr(), adaptor.getIndices());
    rewriter.replaceOp(op, llvmGEP);

    return success();
  }
};

struct SPIRVVectorInsertDynamicConversion
    : public ConvertOpToLLVMPattern<spirv::VectorInsertDynamicOp> {
  using ConvertOpToLLVMPattern<
      spirv::VectorInsertDynamicOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(spirv::VectorInsertDynamicOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    auto vecTy = getTypeConverter()->convertType(op.getType());

    Value llvmIE = rewriter.create<LLVM::InsertElementOp>(
        loc, vecTy, adaptor.getVector(), adaptor.getComponent(),
        adaptor.getIndex());

    rewriter.replaceOp(op, llvmIE);

    return success();
  }
};

class SPIRVCompositeExtractConversion
    : public ConvertOpToLLVMPattern<spirv::CompositeExtractOp> {
public:
  using ConvertOpToLLVMPattern<
      spirv::CompositeExtractOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(spirv::CompositeExtractOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    llvm::outs() << "johnlu CompositeExtractOp: " << op << "\n";
    llvm::outs().flush();
    ...
    if (isa<VectorType>(containerType)) {
      ...
      Value index = rewriter.create<LLVM::ConstantOp>(
          loc, i32ty, IntegerAttr::get(i32ty, value.getInt()));
...
void populateSPIRVToLLVMPatterns(LLVMTypeConverter &typeConverter,
                                 ...
                                 PatternBenefit benefit) {
  patterns.add<SPIRVAddressOfConversion>(typeConverter, benefit);
  patterns.add<SPIRVUndefConversion>(typeConverter, benefit);
  patterns.add<SPIRVPtrAccessChainConversion>(typeConverter, benefit);
  patterns.add<SPIRVVectorInsertDynamicConversion>(typeConverter, benefit);
  patterns.add<SPIRVCompositeExtractConversion>(typeConverter, benefit);
}
