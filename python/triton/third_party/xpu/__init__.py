...
from triton._C.libintel_xpu_backend_for_triton import ir, runtime
from triton._C.libintel_xpu_backend_for_triton.translation import ClusterInfo, get_num_warps, TMAInfos, translate_triton_gpu_to_llvmir, get_shared_memory_size, get_threads_per_warp, add_external_libs, translate_llvmir_to_spirv
...
class XPUBackend(BaseBackend):
    ...
    def make_llir(src, metadata, options):
        ...
        # TritonGPU -> LLVM-IR
        ret = translate_triton_gpu_to_llvmir(src, 0, TMAInfos(), runtime.TARGET.GENX)
        metadata["shared"] = get_shared_memory_size(src)
        metadata["threads_per_warp"] = get_threads_per_warp(src)
        metadata["cluster_dims"] = (1, 1, 1)
        return ret

    @staticmethod
    def make_spv(src, metadata, opt):
    ...
    def parse_options(self, _opts, target) -> Any:
        ...
        capability = target[1]
        threads_per_warp = args['threads_per_warp'] if 'threads_per_warp' in args else 32
