...
Value loadC(Value tensor, Value llTensor,
  ...
  auto structTy = llTensor.getType().cast<spirv::StructType>();
  ...
  if (numMmaRets == 4) {
    ...
    for (int i = 0; i < fcSize; i += numCPackedElem) {
      ...
      for (int j = 0; j < numCPackedElem; ++j) {
        pack = insert_element(
            cPackTy, pack,
            extract_val(cElemTy, llTensor, rewriter.getI32ArrayAttr(i + j)),
            i32_val(j));
      }
...
//      funValue.getPointer().print(llvm::outs());
  ...
  auto callMma = [&](unsigned m, unsigned n, unsigned k) {
    ...
    //    auto retArgs =
    //        builder.newListOperand(numMmaRets, isIntMMA || isAccF16 ? "=r" :
    //        "=f");
    ...
    //    });
    //    auto bArgs =
    //        builder.newListOperand({{hb[{n, k}], "r"}, {hb[{n, k + 1}],
    //        "r"}});
    ...
    //    Value mmaOut =
    //        builder.launch(rewriter, loc, getMmaRetType(mmaType,
    //        op.getContext()));
    //
    //    Type elemTy =
    //    mmaOut.getType().cast<LLVM::LLVMStructType>().getBody()[0]; for (int i
    //    = 0; i < numMmaRets; ++i) {
    //      fc[(m * colsPerThread + 4 * n) / numCPackedElem + i] =
    ...
    for (int i = 0; i < 4; ++i)
      fc[m * colsPerThread + 4 * n + i] =
          rewriter.create<spirv::UndefOp>(loc, elemTy);
  };
  ...
  Type structTy = spirv::StructType::get(
      SmallVector<Type>(fc.size() * numCPackedElem, resElemTy));
